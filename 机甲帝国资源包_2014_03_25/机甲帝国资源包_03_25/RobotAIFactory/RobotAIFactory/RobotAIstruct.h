...
struct RobotAI_RobotInformation
{
	...
	double engineRotation;	//引擎角度，与x轴正方向的夹角
	double weaponRotation;	//武器角度，与x轴正方向的夹角
...
struct RobotAI_BulletInformation
{
	...
	Circle circle;		//子弹的圆形(结构体：x,y,radium=0)
...
struct RobotAI_BattlefieldInformation
{
	...
	int num_robot;	//机甲数量（本届争霸赛恒为2了，包括已经挂了的）
